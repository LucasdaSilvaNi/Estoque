...
{
    public class ImportacaoUA : ImportacaoCargaControle, ICarga
    {
        public bool InsertImportacao(Infrastructure.TB_CONTROLE entityList)
        {
            try
            {
                bool isErro = new UaBusiness().InsertListControleImportacao(entityList);
        ...
        public TB_CENTRO_CUSTO ValidaCentroDeCusto(TB_CARGA carga)
        {
            ...
            if (carga.TB_CENTRO_CUSTO_CODIGO != null)
            {
                var lObjCentroCusto = new CentroCustoBusiness().SelectWhere(centroCusto => centroCusto.TB_CENTRO_CUSTO_CODIGO == carga.TB_CENTRO_CUSTO_CODIGO
        ...
        public TB_UA ValidaUA(TB_CARGA carga)
        {
            ...
            if (uaCodigo != null)
            {
                var lObjUa = new UaBusiness().SelectWhere(Ua => Ua.TB_UA_CODIGO == uaCodigo
                    && Ua.TB_GESTOR_ID == carga.TB_GESTOR_ID).FirstOrDefault();
        ...
        public TB_UGE ValidaUGE(TB_CARGA carga)
        {
            ...
            if (ugeCodigo != null)
            {
                var lObjUGE = new UgeBusiness().SelectWhere(Uge => Uge.TB_UGE_CODIGO == ugeCodigo).FirstOrDefault();
        ...
        public TB_RESPONSAVEL ValidaResponsavel(TB_CARGA carga)
        {
            ...
            long? responsavelCodigo = TratamentoDados.TryParseInt32(carga.TB_RESPONSAVEL_CODIGO);
        ...
        public TB_GESTOR ValidaGestor(TB_CARGA carga)
        {
            ...
            int? almoxId = carga.TB_ALMOXARIFADO_ID;

            if (!String.IsNullOrEmpty(carga.TB_GESTOR_NOME_REDUZIDO) && almoxId != null)
            {
                var lObjAlmox = new AlmoxarifadoBusiness().SelectWhere(almox => almox.TB_ALMOXARIFADO_ID == almoxId).FirstOrDefault();

                if (lObjAlmox != null && lObjAlmox.TB_ALMOXARIFADO_ID != 0)
                {
                    var lObjGestor = new GestorBusiness().SelectWhere(gestor => (gestor.TB_GESTOR_NOME_REDUZIDO == nomeGestor)
                        && gestor.TB_GESTOR_ID == lObjAlmox.TB_GESTOR_ID).FirstOrDefault();
