...
namespace Sam.Integracao.SIAF.Core
{
    ...
    public class ProcessadorServicoSIAF
    {
        ...
        private bool erroAcessoOuConexao(string strRetornoMsgEstimulo, out string strMensagemErro)
        {
            ...
            msgErroRetornoWs = strMensagemErro = strRetorno;
            erroProcessamentoEstimulo = blnRetorno;
        ...
        private bool horarioAcessoInvalido(string strRetornoMsgEstimulo)
        {
            ...
            if (docXml.IsNotNull())
            {
                ...
                msgErroRetornoWs = (docXml.GetElementsByTagName(nomeTagMsgErro).Cast<XmlNode>().FirstOrDefault() ?? docXml.GetElementsByTagName(nomeTagMsgRetorno).Cast<XmlNode>().FirstOrDefault()).InnerText;
            }
        ...
        private bool verificarErroMensagem(string strRetornoMsgEstimulo)
        {
            ...
            nomeMsgEstimuloWs = docXml.GetElementsByTagName("cdMsg").Cast<XmlNode>().FirstOrDefault().NextSibling.Name;
            nomeSistemaSIAF = ((nomeMsgEstimuloWs.Contains("Siafisico") || nomeMsgEstimuloWs.Contains("SFCODOC")) ? "SIAFISICO" : "SIAFEM");
            ...
            //if (!string.IsNullOrEmpty(String.Format(hdrRetornoMsgErro, docXml.GetElementsByTagName("NumeroNL").Cast<XmlNode>().FirstOrDefault().InnerText, nomeSistemaSIAF).Trim()))
