...
{
    public class ConsumoImediatoPresenter : CrudPresenter<IConsumoImediatoView>
    {

        IConsumoImediatoView view;
        private MovimentoBusiness estrutura = null;
        string statusRetornoSIAF_SAM = null;
        public IConsumoImediatoView View
        {
            get { return view; }
            set { view = value; }
        }

        public ConsumoImediatoPresenter(IConsumoImediatoView _view)
            : base(_view)
        {
            this.View = _view;
        }

        public ConsumoImediatoPresenter()
        {
        }
        ...
        public void CarregarMovimentoTela(MovimentoEntity mov)
        {
            ...
            this.View.Id = ((mov.Id.HasValue) ? mov.Id.Value.ToString() : null);
           ...
           // this.View.InscricaoCE = mov.InscricaoCE;
            ...
            if (mov.UA.IsNotNull() && mov.UA.Uge.IsNotNull())
                this.View.UgeId = mov.UA.Uge.Id.Value;
            ...
            if ((this.View.TipoOperacao == (int)TipoOperacao.NotaRecebimento) || (this.View.TipoOperacao == (int)TipoOperacao.Estorno))
            {
                ...
                int codigoPtres = -1;
                if (!String.IsNullOrWhiteSpace(mov.MovimentoItem[0].PTResCodigo) && Int32.TryParse(mov.MovimentoItem[0].PTResCodigo, out codigoPtres) && (codigoPtres > 0))
                {
                    this.View.PTResAcao = mov.MovimentoItem[0].PTResAcao;
                    this.View.PTResCodigo = codigoPtres;
                }

                if ((mov.UA.IsNotNull() && mov.UA.Id.HasValue) &&
                    (mov.UA.Uge.IsNotNull() && mov.UA.Uge.Id.HasValue))
                {
                    this.View.UgeId = mov.UA.Uge.Id.Value;
                    this.View.UaId = mov.UA.Id.Value;
            ...
            if ((this.View.TipoOperacao == (int)TipoOperacao.NotaRecebimento) && ((mov.TipoMovimento.Id == (int)TipoMovimento.ConsumoImediatoEmpenho) || (mov.TipoMovimento.Id == (int)TipoMovimento.EntradaPorRestosAPagarConsumoImediatoEmpenho)))
            {
                this.View.ExibirListaEmpenho = false;
                this.View.ExibirNumeroEmpenho = true;
                this.View.Empenho = mov.Empenho;
            }
            else if (this.View.TipoOperacao == (int)TipoOperacao.Nova)
            {
                this.View.ExibirListaEmpenho = true;
            }
        }
        ...
        public override void Load()
        {
            ...
            //this.View.BloqueiaNovo = (this.View.TipoMovimento != TipoMovimento.EntradaPorRestosAPagarConsumoImediatoEmpenho.GetHashCode());
            this.View.PopularListaTipoMovimentoConsumoImediato();
        ...
        public IList<string> ObterListaEmpenhos(int almoxarifadoId, int gestorId, int codigoUGE, string loginSiafemUsuario, string senhaSiafemUsuario, bool listarLiquidados = true, bool listarRestosAPagar = false)
        {
            ...
            try
            {
                ...
                //bool ConsumoImediato = true;
                bool isConsumoImediato = !listarRestosAPagar;

                lILstEmpenhos = lObjBusiness.obterListaEmpenhosSiafem(almoxarifadoId, gestorId, codigoUGE, almoxMesRef, loginSiafemUsuario, senhaSiafemUsuario, listarRestosAPagar, isConsumoImediato);
        ...
        public MovimentoEntity GravarItem(MovimentoEntity mov, int flLote, string botao)
        {
            ...
            if (this.View.TipoMovimento != (int)GeralEnum.TipoMovimento.ConsumoImediatoEmpenho)
            ...
            if (botao != "btnEditar")
            {
                if (VerificaSeSubitemInserido(mov, movItem))
                {
                    ...
                    this.View.ExibirMensagem(String.Format("Subitem {0} - {1} inserido anteriormente nesta movimentação de material. Não permitido inserir o mesmo subitem mais de uma vez!", movItem.SubItemMaterial.Codigo, movItem.SubItemMaterial.Descricao));
                    return null;
                }
        ...
        public void HabilitarControles(int tipoMovimento, bool Editar)
        {
            switch (tipoMovimento)
            {
                ...
                case (int)GeralEnum.TipoMovimento.ConsumoImediatoEmpenho:
                    this.View.HabilitarEmpenhoConsumoImediato(Editar);
                    break;
                ...
                case (int)GeralEnum.TipoMovimento.EntradaPorRestosAPagarConsumoImediatoEmpenho:
                    this.View.HabilitarAquisicaoRestosAPagarConsumoImediato(Editar);
                    //this.View.ExibeBotaoNovoItem = true;
                    break;
        ...
        public MovimentoEntity Estornar(string loginSiafemUsuario, string senhaSiafemUsuario, MovimentoEntity mov)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var statusEstornoMovimentacaoSAM = estrutura.EstornarMovimentoConsumo(perfilLogado_ID, this.View.InscricaoCE);
                    if (statusEstornoMovimentacaoSAM.Item1)
                    {
                        tipoMovID = estrutura.Movimento.TipoMovimento.Id;
                        strDescricaoTipoMovimentacao = _obterDescricaoTipoMovimentacao(tipoMovID);
                        ...
                        if (statusEstornoMovimentacaoSIAFEM)
                        {
                            ...
                            if (String.IsNullOrWhiteSpace(this.statusRetornoSIAF_SAM))
                            {
                                ...
                                if(!String.IsNullOrWhiteSpace(nlReclassificacaoMovimentacaoMaterial))
                                    fmtMsgInformeAoUsuario = @"Geradas NLs (estorno) números ""{0}"" (NLEmLiq) e ""{1}"" (Reclassificação - BAIXA) no SIAFEM";
        ...
        public void Imprimir()
        {
            ...
            relatorioImpressao.Id = (int)RelatorioEnum.ConsumoImediato;
            relatorioImpressao.Nome = "AlmoxConsumoImediato.rdlc";
        ...
        public void HabilitarControlesNovo(int tipoMovimento)
        {
            ...
            switch (tipoMovimento)
            {
                //case (int)GeralEnum.TipoMovimento.AquisicaoCompraEmpenho:
                case (int)GeralEnum.TipoMovimento.ConsumoImediatoEmpenho:
                    this.View.HabilitarEmpenhoConsumoImediatoNovo();
                    break;
                case (int)GeralEnum.TipoMovimento.EntradaPorRestosAPagarConsumoImediatoEmpenho:
                    this.View.HabilitarAquisicaoRestosAPagarConsumoImediatoNovo();
                    break;
        ...
        private MovimentoEntity ExecutaEntradaMaterial(string loginSiafemUsuario, string senhaSiafemUsuario, string tipo, MovimentoEntity mov)
        {
            ...
            else if (this.View.TipoMovimento == (int)GeralEnum.TipoMovimento.EntradaPorRestosAPagarConsumoImediatoEmpenho)
            {
                estrutura.Movimento.Empenho = this.View.NumeroEmpenhoCombo;
            }
            ...
            try
            {
                ...
                estrutura.Movimento.InscricaoCE = this.View.InscricaoCE;
                var statusGravacaoMovimentacao = estrutura.GravarMovimentoConsumo();
            ...
            if (estrutura.Movimento.IsNotNull() && estrutura.Movimento.Id.HasValue)
            {
                ...
                else
                {
                    if (String.IsNullOrWhiteSpace(this.statusRetornoSIAF_SAM))
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(nlReclassificacaoMovimentacaoMaterial))
                            fmtMsgInformeAoUsuario = @"Geradas NLs números ""{0}"" (NLEmLiq) e ""{1}"" (Reclassificação - BAIXA) no SIAFEM";
        ...
        public Tuple<decimal,decimal> RecalculoQtdeLiqItemSiafisico(int itemMaterialCodigo, int iUge_ID, string strEmpenhoSiafisicoCodigo, int iAlmoxarifado_ID, int gestorId, int iNaturezaDespesaCodigo, string unidadeFornSiafem, bool consumoImediato)
        {
        ...
        {
            decimal valorSaldoLiquidadoItemEmpenho = (_valorZero);

            EmpenhoBusiness objBusiness = null;

            objBusiness = new EmpenhoBusiness();
            valorSaldoLiquidadoItemEmpenho = objBusiness.RecalculoValorLiquidadoItemEmpenho(iItemMaterialCodigo, ugeID, codigoEmpenho, almoxID, gestorID, codigoNaturezaDespesa, unidadeFornSiafem);


            return valorSaldoLiquidadoItemEmpenho;
        }
